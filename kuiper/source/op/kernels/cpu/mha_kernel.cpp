...
void mha_kernel(int32_t pos, int32_t head_num, int32_t layer_index, int32_t seq_len, int32_t kv_dim,
  ...
  for (int32_t h = 0; h < head_num; ++h) {
    ...
    // 1. 计算 Query 和 Key 的点积，得到 Score
    for (int32_t t = 0; t <= pos; t++) {
      int logical_block_idx = t / block_size;
      int token_offset = t % block_size;
      int physical_block_idx = block_table[logical_block_idx];

      int64_t physical_offset = layer_offset + physical_block_idx * block_size * kv_dim + token_offset * kv_dim + head_offset;
      //int32_t cache_offset = t * kv_dim + (h / kv_mul) * head_size;
      const float* key_head_addr = key_cache_tensor.ptr<float>() + layer_offset + physical_offset;
    ...
    for (int32_t t = 0; t <= pos; t++) {
      ...
      int64_t physical_offset = layer_offset + physical_block_idx * block_size * kv_dim + token_offset * kv_dim + head_offset;
      const float* value_head_addr = value_cache_tensor.ptr<float>() + physical_offset;
      float score = score_head_addr[t];
      for (int32_t i = 0; i < head_size; i++) {
        output_head_ptr[i] += score * value_head_addr[i];
      }
    }
